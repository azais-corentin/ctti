...
struct symbol_mapping {
    ...
    template <typename Source, typename Sink>
    auto operator()(const Source &source, SourceSymbol, Sink &sink, SinkSymbol) const -> ctti::meta::void_t<
        decltype(std::declval<Function>()(ctti::get_member_value<SourceSymbol>(source),
                                          ctti::type_tag<typename SinkSymbol::template value_type<Sink>>()))> {
        ctti::set_member_value<SinkSymbol>(sink,
                                           function(ctti::get_member_value<SourceSymbol>(source),
                                                    ctti::type_tag<typename SinkSymbol::template value_type<Sink>>()));
    }
};

template <typename SourceSymbol, typename SinkSymbol, typename Function>
constexpr ctti::symbol_mapping<SourceSymbol, SinkSymbol, ctti::meta::decay_t<Function>> mapping(
...
template <typename Source, typename Sink, typename... SourceSymbols, typename... SinkSymbols,
          typename... MappingFunctions>
void map(const Source &source, Sink &sink,
         const ctti::symbol_mapping<SourceSymbols, SinkSymbols, MappingFunctions> &...mapping) {
