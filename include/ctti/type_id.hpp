...
struct type_id_t {
    ...
    friend constexpr bool operator==(const type_id_t &lhs, const type_id_t &rhs) { return lhs.hash() == rhs.hash(); }

    friend constexpr bool operator!=(const type_id_t &lhs, const type_id_t &rhs) { return !(lhs == rhs); }
...
template <std::size_t N> constexpr ctti::unnamed_type_id_t id_from_name(const char (&typeName)[N]) {
...
namespace detail {
template <typename T> CTTI_CONSTEXPR_ID ctti::type_id_t type_id() { return {ctti::nameof<T>()}; }

template <typename T> CTTI_CONSTEXPR_ID ctti::unnamed_type_id_t unnamed_type_id() {
 ...
 */
template <typename T> constexpr type_id_t type_id(T &&) { return detail::type_id<typename std::decay<T>::type>(); }

/**
 * Returns type information at compile-time for type T.
 */
template <typename T> constexpr type_id_t type_id() { return detail::type_id<T>(); }
 ...
 */
template <typename T> constexpr unnamed_type_id_t unnamed_type_id() { return detail::unnamed_type_id<T>(); }
