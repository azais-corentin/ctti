...
class cstring {
  public:
    ...
    constexpr hash_t hash() const { return fnv1a_hash(length(), begin()); }

    std::string cppstring() const { return {begin(), end()}; }

    std::string str() const { return cppstring(); }

    constexpr const char *begin() const { return _str; }

    constexpr const char *end() const { return _str + _length; }

    constexpr char operator[](std::size_t i) const { return _str[i]; }

    constexpr const char *operator()(std::size_t i) const { return _str + i; }

    constexpr cstring operator()(std::size_t begin, std::size_t end) const { return {_str + begin, _str + end}; }
...
constexpr bool operator!=(const cstring &lhs, const cstring &rhs) { return !(lhs == rhs); }
