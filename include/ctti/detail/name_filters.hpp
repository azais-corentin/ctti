...
constexpr const char *find_ith_impl(const ctti::detail::cstring &name, const ctti::detail::cstring &substring,
                                    const char *res, std::size_t i, bool infinite = false) {
    return (name.length() >= substring.length())
               ? ((name(0, substring.length()) == substring)
                      ? ((i == 0) ? name.begin()
                                  : find_ith_impl(name(substring.length(), name.length()), substring, name.begin(),
                                                  i - 1, infinite))
...
constexpr const char *find_ith(const ctti::detail::cstring &name, const ctti::detail::cstring &substring,
                               std::size_t i) {
...
constexpr const char *find_last(const ctti::detail::cstring &name, const ctti::detail::cstring &substring) {
    return find_ith_impl(name, substring, name.end(), static_cast<std::size_t>(-1), true);
}
