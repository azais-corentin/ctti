...
template <typename T, T Value> constexpr ctti::detail::cstring nameof();
...
struct TypeNameLength<Enum, typename std::enable_if<std::is_enum<Enum>::value>::type>
    : std::integral_constant<std::size_t, ctti::nameof<Enum>().length()> {};
...
template <typename T, typename = void> struct nameof_impl {
    static constexpr ctti::detail::cstring apply() {
        return ctti::detail::filter_typename_prefix(
            ctti::pretty_function::type<T>().pad(CTTI_TYPE_PRETTY_FUNCTION_LEFT, CTTI_TYPE_PRETTY_FUNCTION_RIGHT));
    }
};

template <typename T> struct nameof_impl<T, ctti::meta::void_t<decltype(T::ctti_nameof())>> {
    static constexpr ctti::detail::cstring apply() { return T::ctti_nameof(); }
};

template <typename T, T Value> struct nameof_impl<another_level_of_indirection<ctti::static_value<T, Value>>, void> {
    static constexpr ctti::detail::cstring apply() {
        return ctti::pretty_function::value<T, Value>().pad(CTTI_VALUE_PRETTY_FUNCTION_LEFT +
                                                                ctti::detail::TypeNameLength<T>::value +
                                                                CTTI_VALUE_PRETTY_FUNCTION_SEPARATION,
                                                            CTTI_VALUE_PRETTY_FUNCTION_RIGHT);
    }
};

template <typename T, T Value> struct nameof_impl<ctti::static_value<T, Value>, void> {
    static constexpr ctti::detail::cstring apply() { return ctti::nameof<T, Value>(); }
};

} // namespace detail

template <typename T> constexpr ctti::detail::cstring ctti_nameof(ctti::type_tag<T>) {
    return ctti::detail::nameof_impl<T>::apply();
}

template <typename T, T Value> constexpr ctti::detail::cstring ctti_nameof(ctti::static_value<T, Value>) {
...
template <typename T> constexpr ctti::detail::cstring nameof_v = ctti::nameof<T>();

// CONSIDER USING nameof_v<CTTI_STATIC_VALUE(value)> INSTEAD
template <typename T, T Value> constexpr ctti::detail::cstring nameof_value_v = ctti::nameof<T, Value>();
#endif // CTTI_HAS_VARIABLE_TEMPLATES

} // namespace ctti

namespace std {
constexpr ctti::detail::cstring ctti_nameof(ctti::type_tag<std::string>) { return "std::string"; }
} // namespace std
